...
WPGenerator.prototype.getVersion = function getVersion() {
	var cb = this.async(),
		self = this,
		latestVersion = '3.5.1',
		version; // we still store the latest version to avoid throwing error
	...
	try {
		version = exec('git ls-remote --tags git://github.com/WordPress/WordPress.git | tail -n 1', function(err, stdout) {
					  if (err) {
...
WPGenerator.prototype.getBootstrapVersion = function getBootstrapVersion() {
	...
	// try to get the latest version using the git tags
	try {
		version = exec('git ls-remote --tags git://github.com/twitter/bootstrap.git | tail -n 1', function(err, stdout) {
					  if (err) {
...
WPGenerator.prototype.getFAVersion = function getFAVersion() {
	...
	// try to get the latest version using the git tags
	try {
		version = exec('git ls-remote --tags git://github.com/FortAwesome/Font-Awesome.git | tail -n 1', function(err, stdout) {
					  if (err) {
...
WPGenerator.prototype.askFor = function askFor() {
	...
	this.prompt(prompts, function(e, props) {
		...
		if (!tarballLink) {
			...
			lastChar = self.themeBoilerplate.substring(self.themeBoilerplate.length - 1);
			if (lastChar === '/') {
...
WPGenerator.prototype.bootAwesome = function bootAwesome() {
	...
	pathFile = 'src/less/includes/bootstrap.less';
...
WPGenerator.prototype.createDatabase = function createDatabase() {
	...
	function buildDB() {
		...
		connection.connect(function(err) {
			...
			connection.query('CREATE DATABASE ' + self.dbtable, function(err) {
				if (err) {
...
WPGenerator.prototype.buildStylesheet = function buildStylesheet() {
	...
	grunt = spawn('grunt', ['less'], { stdio: 'inherit' });
